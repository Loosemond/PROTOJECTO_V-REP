...
clientID=vrep.simxStart('192.168.1.12',19999,True,True,5000,5) # Connect to V-REP
if clientID!=-1:
...
def Sensores_check(clientID,sensor1):
    ...
    detectedPointx[0]=detectedPoint1

    print(detectedPointx[0])
...
#Variaveis em paralelo
detectedPointx = Array('d', [0]*16)
detectedPointy = Array('d', [0]*16)
detectedPointz = Array('d', [0]*16)
detectionState = Array('i', [0] *16)


#iniciaçao de multi
olhos=Process(target=Sensores_check,args=(clientID,sensor1))
olhos.start()

while time.time()-startTime < 120 : #loop
    time.sleep(0.05)
    olhos.join()
    Process(target=Sensores_check, args=(clientID, sensor1)).start() # Muito cuidado pois é possivel que se comesse um novo processo sem que este tenha acabdo       !!!PERIGO!!!
    # print(detectedPointx[0])
    skip=False
    errorCode,detectionState1,detectedPoint1,detectedObjectHandle,detectedSurfaceNormalVector=vrep.simxReadProximitySensor(clientID,sensor1,vrep.simx_opmode_buffer)
